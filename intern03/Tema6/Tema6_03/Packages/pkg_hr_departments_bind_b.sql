...
BEGIN
    ...
    USING an_deptid, as_deptname, an_managid, an_locid;
...
BEGIN
    EXECUTE IMMEDIATE 'SELECT 	1 
                       FROM 	departments
                       WHERE 	department_id = :1' INTO 	ln_dep
    USING an_deptid;  
...
IS
    ln_dep NUMBER;
BEGIN
    EXECUTE IMMEDIATE 'SELECT department_id 
                       FROM 	departments 
                       WHERE 	department_id = :1' INTO 	ln_dep
    USING an_deptid;
...
IS
BEGIN
       EXECUTE IMMEDIATE 'UPDATE departments
                          SET department_name  = :1,
                          manager_id       = :2,
                          location_id      = :3
                          WHERE department_id  = :4'
       USING as_deptname, an_managid, an_locid,an_deptid;
